...
command = 'cat '+result_dir+'/*/prediction | grep " [0-9]" | sed \'s/\*//g;s/+//g;s/[\ ][\ ]*/\t/g;s/[0-9]://g\' | cut -f3-5 | sed \'s/,/\t/g\' >' + result_dir +'/../results'
print command
system(command)

# for the unselected features
results = numpy.loadtxt(result_dir+'/../results')


# remake resuls based on probabilities
class0 = results[:,2]>0.33

class1or2 = results[:,2]<0.33
class1 = results[:,3] > results[:,4]
class2 = results[:,4] > results[:,3]
